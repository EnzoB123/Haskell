...
somaDigit :: Int -> Int
somaDigit x = if x == 0 then 0 else mod x 10 + somaDigit (div x 10)

{- 4. Escreva  uma  função  que  devolva  a  soma  de  todos  os  números  menores  que  10000  que sejam múltiplos de 3 ou 5. -}
menoresDezK :: Int -> Int -> Int
menoresDezK a xs | a >= 10000 = xs
                     | mod a 3 == 0 || mod a 5 == 0 = a + menoresDezK (a + 1) xs
                     | otherwise = menoresDezK (a + 1) xs
...
somaDiff :: [Int] -> Int -> Int
somaDiff x y | null (tail x) = head x ^ y
             | otherwise = (head x) ^ y + somaDiff (tail x) y

soma :: [Int] -> Int
soma x = somaDiff x 2 - (somaDiff x 1) ^ 2
...
lucasSq (0) = 2
lucasSq (1) = 1
lucasSq n = lucasSq (n - 2) + lucasSq (n - 1)

lucas :: Int -> [Int]
lucas n = [lucasSq (x) | x <- [0..n]]
...
main = do
  putStrLn ("Func. 1: entrada:7; resultado:" ++ show (fibo 7))
  putStrLn ("Func. 2: entrada:22 12; resultado:" ++ show (mdc 22 12))
  putStrLn ("Func. 3: entrada:12345; resultado:" ++ show (somaDigit 12345))
  putStrLn ("Func. 4: entrada:0 0; resultado:" ++ show (menoresDezK 0 0))
  putStrLn ("Func. 5: entrada:[2,2,2]; resultado:" ++ show (soma [2, 2, 2]))
  putStrLn ("Func. 6: entrada:51; resultado:" ++ show (primo 51))
